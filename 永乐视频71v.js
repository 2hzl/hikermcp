...
const qy71v = {
    ...
    /* ===== 基础工具 ===== */
    hdr: function() {
        return { headers: { "User-Agent": this.ua, "Referer": this.host + "/" } };
    },
    full: function(u) {
        if (!u) return '';
    ...
    /* ===== 统一列表解析 ===== */
    /* 71v.net(AppleCMS)：<a href="/voddetail/{id}/">标题</a>，
       同卡片常出现"正片/第X集"状态链接，封面 data-original 懒加载 */
    ...
    home: function() {
        ...
        if (!cat) {
            ...
            html = self.fetch(self.host + '/vodshow/' + cat + '-----------' + (pg > 1 ? pg : '') + '/');
        }
        var list = self.parseList(html || '');
        if (!list.length && pg == 1) d.push({ title: '暂无数据（可能被CF风控，稍后再试）', col_type: 'text_center_1' });
    ...
    search: function() {
        ...
        if (pg == 1) {
            ...
            url = self.host + '/vodsearch/' + encodeURIComponent(kw) + '----------' + pg + '---/';
        }
        var html = self.fetch(url);
        var list = self.parseList(html || '');
        if (!list.length && pg == 1) d.push({ title: '没有找到"' + kw + '"相关影片', col_type: 'text_center_1' });
    ...
    detail: function() {
        ...
        if (h1) tTitle = h1[1].replace(/<[^>]+>/g, '').replace(/^\s+|\s+$/g, '') || tTitle;
        setPageTitle(tTitle);
        ...
        for (var oi = 0; oi < order.length; oi++) {
            ...
            for (var ai = 0; ai < arr.length; ai++) {
                ...
                eps.push({ name: arr[ai].name, sid: sk, nid: arr[ai].nid, vid: arr[ai].vid });
            }
    ...
    play: function(sid, nid, vid) {
        var self = this;
        var pu = self.host + '/play/' + vid + '-' + sid + '-' + nid + '/';
